...
void __init mx50_yoshime_io_init(void)
{
	...
	if(mx50_board_rev_greater(BOARD_ID_CELESTE_EVT1_2) ||
		...
		mx50_board_rev_greater_eq(BOARD_ID_CELESTE_WFO_512_EVT3) ||
		mx50_board_rev_greater_eq(BOARD_ID_CELESTE_256_EVT3) ||
		mx50_board_is(BOARD_ID_ICEWINE) || mx50_board_is(BOARD_ID_ICEWINE_WFO)) {
...
#ifdef CONFIG_SUMMIT_SMB242
	if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
		/* smb232 susp pin */
		mxc_request_iomux(MX50_PIN_DISP_BUSY, IOMUX_CONFIG_ALT1);
		mxc_iomux_set_pad(MX50_PIN_DISP_BUSY, PAD_CTL_PKE_ENABLE |
		...
		/* smb232 en pin */
		mxc_request_iomux(MX50_PIN_DISP_RESET, IOMUX_CONFIG_ALT1);
		mxc_iomux_set_pad(MX50_PIN_DISP_RESET, (PAD_CTL_PKE_NONE | 
						PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_DRV_HIGH));
        gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_DISP_RESET), 1);

		/* smb232 intr pin */ 
		mxc_request_iomux(MX50_PIN_DISP_WR, IOMUX_CONFIG_ALT1);
		mxc_iomux_set_pad(MX50_PIN_DISP_WR, (PAD_CTL_PKE_NONE | 
						PAD_CTL_ODE_OPENDRAIN_ENABLE | PAD_CTL_HYS_ENABLE));
		gpio_request(IOMUX_TO_GPIO(MX50_PIN_DISP_WR), "smb_intr");
		gpio_direction_input(IOMUX_TO_GPIO(MX50_PIN_DISP_WR));

	} else if (mx50_board_is(BOARD_ID_ICEWINE_PROTO)) { 
		/* smb232 susp pin */
        mxc_request_iomux(MX50_PIN_KEY_COL2, IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX50_PIN_KEY_COL2, PAD_CTL_PKE_ENABLE |
                        PAD_CTL_PUE_PULL | PAD_CTL_47K_PU |
                        PAD_CTL_DRV_HIGH | PAD_CTL_ODE_OPENDRAIN_ENABLE);
        gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_KEY_COL2), 0);

		/* smb232 en pin */
		mxc_request_iomux(MX50_PIN_KEY_COL3, IOMUX_CONFIG_ALT1);
		mxc_iomux_set_pad(MX50_PIN_KEY_COL3, (PAD_CTL_PKE_NONE | 
						PAD_CTL_ODE_OPENDRAIN_NONE | PAD_CTL_DRV_HIGH));
        gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_KEY_COL3), 1);

		/* smb232 intr pin */
		mxc_request_iomux(MX50_PIN_KEY_ROW3, IOMUX_CONFIG_ALT1);
		mxc_iomux_set_pad(MX50_PIN_KEY_ROW3, (PAD_CTL_PKE_NONE | 
						PAD_CTL_ODE_OPENDRAIN_ENABLE | PAD_CTL_HYS_ENABLE));
	}
#endif
...
#ifdef CONFIG_SUMMIT_SMB242     
void gpio_smb242_chgsusp_pin(int enable)
{
	if (enable > 0) {
		if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
			gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_DISP_BUSY), 1);
		} else {
			gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_KEY_COL2), 1);
		}
	} else {
		if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
			gpio_direction_output(IOMUX_TO_GPIO(MX50_PIN_DISP_BUSY), 0);
...
EXPORT_SYMBOL(gpio_smb242_chgsusp_pin);

void gpio_smb242_charger_enable(int enable)
{
	if (enable > 0) {
		if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
			gpio_set_value(IOMUX_TO_GPIO(MX50_PIN_DISP_RESET), 0);
		} else {
			gpio_set_value(IOMUX_TO_GPIO(MX50_PIN_KEY_COL3), 1);
		}
	} else {
		if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
			gpio_set_value(IOMUX_TO_GPIO(MX50_PIN_DISP_RESET), 1);
		} else {
			gpio_set_value(IOMUX_TO_GPIO(MX50_PIN_KEY_COL3), 0);
		}
	}
}
EXPORT_SYMBOL(gpio_smb242_charger_enable);

int gpio_smb242_int(void)
{
	if(mx50_board_rev_greater(BOARD_ID_ICEWINE_PROTO)) {
		return IOMUX_TO_IRQ(MX50_PIN_DISP_WR);
	} else {
		return IOMUX_TO_IRQ(MX50_PIN_KEY_ROW3);
	}
}
EXPORT_SYMBOL(gpio_smb242_int);
#endif
