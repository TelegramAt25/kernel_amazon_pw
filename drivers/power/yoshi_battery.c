...
#define BATTERY_SUSPEND_CURRENT_ICEWINE 	    48  
#define BATTERY_SUSPEND_CURRENT_ICEWINE_UAH 	850
//end FIXME
...
//FIXME is it 400 for icewine?
#define ICE_BATT_MAH		400
...
#if 0 /* Icewine battery ids are not known yet for proto hw */
	} else if (mx50_board_is(BOARD_ID_ICEWINE) || mx50_board_is(BOARD_ID_ICEWINE_WFO)) {
		list = icewine_valid_ids;
		num = sizeof(icewine_valid_ids)/sizeof(icewine_valid_ids[0]);
#endif
	} else {
...
static void battery_handle_update_nac(struct work_struct *work)
{
	...
	*/
	...
	if (mx50_board_is(BOARD_ID_TEQUILA)) {
		...
		battery_mah = ICE_BATT_MAH;
		battery_suspend_ct = BATTERY_SUSPEND_CURRENT_ICEWINE;
		battery_suspend_uah = BATTERY_SUSPEND_CURRENT_ICEWINE_UAH;
	}
