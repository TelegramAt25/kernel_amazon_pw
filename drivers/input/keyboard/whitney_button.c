...
static long whitney_keys_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		{
			if (copy_to_user(argp, &whitney_button_lock, sizeof(whitney_button_lock)))
				return -EFAULT;
			break;
		}
		...
		default:
			printk(KERN_ERR "whitney-button: Unknown ioctl\n");
			ret = -ENOTTY;
			break;
	}
	return ret;
...
static ssize_t whitney_keys_write_not_implemented(struct file *file, const char __user *buf, 
		size_t count, loff_t *pos)
{ 
	return 0; 
}   

static ssize_t whitney_keys_read_not_implemented(struct file *file, char __user *buf,
		size_t count, loff_t *pos)
{ 
	return 0;
}
...
static const struct file_operations whitney_keys_fops =
{
  .owner = THIS_MODULE,
  .read  = whitney_keys_read_not_implemented,
  .write = whitney_keys_write_not_implemented,
  .unlocked_ioctl = whitney_keys_ioctl,
};
  
static struct miscdevice whitney_keys_misc_device =
{   
  .minor = 167,
  .name  = KEYPAD_MISC_DEV_NAME,
  .fops  = &whitney_keys_fops,
};
...
static int __devinit whitney_keys_probe(struct platform_device *pdev)
{
	...
	if (misc_register(&whitney_keys_misc_device))
	{
		dev_err(&pdev->dev, "whitney-button: Could not register misc device\n");
		return -ENOMEM; 
	}
	/*end proc entry*/
	return 0;
...
static int __devexit whitney_keys_remove(struct platform_device *pdev)
{
	...
	misc_deregister(&whitney_keys_misc_device);
