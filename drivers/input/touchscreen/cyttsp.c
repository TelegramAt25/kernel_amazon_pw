...
static long cyttsp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		{
			int lock = (int)atomic_read(&touch_locked);
			if (copy_to_user(argp, &lock, sizeof(lock)))
				return -EFAULT;
			break;
		}
		...
		{
			int lock;
			if (copy_from_user(&lock, argp, sizeof(int)))
				return -EFAULT;
			if (lock)
			{
				if (!atomic_read(&touch_locked)) {

					int sts = cyttsp_touch_ctrl(cyttsp_tsc, false);
					if (sts < 0){
						atomic_set(&touch_locked, false);
						DEBUG_ERR("cyttsp_proc_write:proc:command=lock:not succeed please retry\n");
						mutex_unlock(&proc_mutex);
						return -EBUSY;
					}
					atomic_set(&touch_locked, true);
			...
			else
			{
				if (atomic_read(&touch_locked)) {
					...
					int sts = cyttsp_touch_ctrl(cyttsp_tsc, true);
					if (sts < 0) {
						atomic_set(&touch_locked, true);
						DEBUG_ERR("cyttsp_proc_write:proc:command=unlock:not succeed please retry\n");
						mutex_unlock(&proc_mutex);
						return -EBUSY;
					}
