...
static void epdc_submit_work_func(struct work_struct *work)
{
	...
	}else if (upd_data_list->update_desc->upd_data.temp == TEMP_USE_AUTO) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			papyrus_temp);
				   ...
				   struct fb_info *info)
{
	...
	if (upd_data->update_marker != 0) {
		...
		if (mxc_epdc_debugging) {
			...
			printk(KERN_INFO "mxc_epdc_fb: [%d] waveform=0x%x mode=0x%x temp:%d update region top=%d, left=%d, width=%d, heigth=%d, temp_override: %d\n", 
	...
	if (fb_data->temp_override != TEMP_USE_AUTO) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			fb_data->temp_override);
	}else if (upd_desc->upd_data.temp == TEMP_USE_AUTO) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			papyrus_temp);
...
static void epdc_intr_work_func(struct work_struct *work)
{
	...
	if (fb_data->temp_override != TEMP_USE_AUTO) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			fb_data->temp_override);
	}else if (fb_data->cur_update->update_desc->upd_data.temp == TEMP_USE_AUTO) {
		temp_index = mxc_epdc_fb_get_temp_index(fb_data,
			papyrus_temp);
...
/*
 * echo 4097 to restore to use papyrus temperature, or echo any meaningful temp to override it. 
 * */
...
*/
	...
	if (device_create_file(&pdev->dev, &dev_attr_mxc_epdc_temperature) < 0)
		dev_err(&pdev->dev, "Unable to create  mxc_epdc_temp_override file\n");
