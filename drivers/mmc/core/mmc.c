...
static int mmc_read_ext_csd(struct mmc_card *card)
{
	...
	if (ext_csd_struct >= 2) {
		...
		/* Add support for Byte addressed cards */
	...
	struct mmc_card *oldcard)
{
	...
	if (oldcard) {
		...
		case SAMSUNG_MANF_ID:
			printk(KERN_INFO "emmc: I def:mmcpartinfo:vendor=samsung, ddr=0, host=%s:\n", mmc_hostname(host));
			host->ddr = 0;
			break;
		case SANDISK_MANF_ID:
			printk(KERN_INFO "emmc: I def:mmcpartinfo:vendor=sandisk, ddr=1, host=%s:\n", mmc_hostname(host));
			host->ddr = 1;
			break;
		case TOSHIBA_MANF_ID:
			printk(KERN_INFO "emmc: I def:mmcpartinfo:vendor=toshiba, ddr=1, host=%s:\n", mmc_hostname(host));
			host->ddr = 1;
			break;
		case HYNIX_MANF_ID:
			printk(KERN_INFO "emmc: I def:mmcpartinfo:vendor=hynix, ddr=0, host=%s:\n", mmc_hostname(host));
			host->ddr = 0;
			break;
		default:
			printk(KERN_INFO "emmc: I def:mmcpartinfo:vendor=unknown, ddr=0, host=%s, manufacturer_id=%lu:\n",
