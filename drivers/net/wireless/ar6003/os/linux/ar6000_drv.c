...
#ifdef LAB126    
/* PCBSN -X field indicates that this is a WAN device converted to WiFi 
 * and hence load the WAN calibration file */
int mx50_is_converted_wifi_device()
{
    if (mx50_board_is(BOARD_ID_WHITNEY_WFO) && mx50_pcbsn_x() == '1') {
        printk(KERN_INFO "kernel: I perf:wifi:Converted WiFi device. PCBSN X is 1");
        return 1;
    } 
    return 0;    
}
...
#define LAB126_DIV_INACTIVE_RATE 36     /* phy rate at which firmware will stop checking other ant */
        ...
        if (enable_diversity && (mx50_board_is(BOARD_ID_TEQUILA) || mx50_board_is(BOARD_ID_WHITNEY_WFO) || 
				mx50_board_is_celeste()	|| mx50_board_is_celeste_wfo() )) {
